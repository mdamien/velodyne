...
#define OHCI1394_MAX_AT_REQ_RETRIES       0x1
#define OHCI1394_MAX_AT_RESP_RETRIES      0x1
#define OHCI1394_MAX_PHYS_RESP_RETRIES    0x2
#define OHCI1394_MAX_SELF_ID_ERRORS       16

#define AR_REQ_NUM_DESC                   4 /* number of AR req descriptors */
#define AR_REQ_SPLIT_BUF_SIZE     4096      /* split packet buffer */
#define INPUT_MORE_CONTEXT_SIZE               16 /* the number of byte for the receive context block (used to alloc memory )*/ 
#define SIZE_BLOCK_RECEIVE               20 /* the number of byte for managing a received message (used to alloc memory )*/ 
...
struct dma_trm_ctx {
  void *ohci;
  quadlet_t *last_descriptor_block;
  int        branch_address_Z_offset;
  //	int ctrlClear;
  //	int ctrlSet;
  //	int cmdPtr;
};
...
struct ti_ohci {
  ...
  struct pci_dev *dev; // low level pointer

  void *les; // hight level pointer really l_e_s_OHCI
  //        u32 state;
  ...
  int self_id_errors;
  int NumBusResets;
  // semaphore to active hight level tasks:
  SEM *irq_sem;
  // time management
  unsigned long NB_64sec ;
};

extern int cross_bound(unsigned long addr, unsigned int size);

/*
 * Register read and write helper functions.
 */
extern void reg_write(const struct ti_ohci *ohci, int offset, u32 data);
extern u32 reg_read(const struct ti_ohci *ohci, int offset);
...
extern void *rt_pci_alloc_consistent( struct pci_dev *dev, int size, unsigned int *dma_handle);
extern void rt_pci_free_consistent( struct pci_dev *dev, int size, void *cpu_addr, unsigned int dma_handle);

extern unsigned int rt_pci_map_single_todevice( struct pci_dev *dev, void *buf, int size);
extern void rt_pci_dma_sync_single( struct pci_dev *dev, unsigned int buf_handle, int size);
extern void rt_pci_unmap_single_todevice( struct pci_dev *dev, unsigned int dma_handle , int size);
extern void enable_irq_dev( struct pci_dev *dev );
